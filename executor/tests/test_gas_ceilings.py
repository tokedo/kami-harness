...
OBSERVED = {
    ...
    # Measured 2026-08-28 over 329,709 successful system.kami.use.item
    # txs since 2026-06-01: p50 1,361,543 / p95 2,185,084 / p99
    # 2,203,762 / max 2,639,799. Floor 3,305,643 = the table's own 1.5x
    # p99, which feed_kami's 3,500,000 clears — the same value
    # travel_use_item carries for the SAME system id, so one system has
    # one ceiling. A 1.3x-p95 floor (2,840,609) was considered during
    # the 3.5.0 build and rejected at gate-2 review: it admits
    # 3,000,000, which is 1.14x the observed max.
    "feed_kami": (3_305_643, 1_361_543, "p99 2,203,762 / max 2,639,799"),
